...
function createContainer(containerData) {
  ...
  container.innerHTML = `
    ...
    <div class="mt-4">
      <label class="block text-sm font-medium mb-2 text-gray-700">Durasi Stream (menit)</label>
      <input 
        type="number" 
        class="stream-duration text-sm w-full p-2 border rounded-lg" 
        min="1"
        max="1440"
...
function addStartStream(container) {
  ...
  async function startStream(formData, duration, streamKey) {
    try {
      ...
      // Pastikan parameter duration dikirim langsung
      const data = {
        ...
        duration: duration // Kirim durasi langsung
      };
      ...
      eventSource.onmessage = (event) => {
        ...
        console.log('Stream status:', status); // Debugging log
