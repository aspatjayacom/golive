...
app.post('/start-stream', async (req, res) => {
  try {
    ...
    const command = ffmpeg(streamFilePath)
      .inputFormat('mp4')
      .inputOptions(['-nostdin', '-re', ...(loop === 'true' ? ['-stream_loop -1'] : [])])
      .outputOptions([
        '-threads 0',
        '-c:v copy',
        '-c:a copy',
    ...
    if (duration) {
      ...
      console.log(`Duration received: ${duration} minutes (${durationMs} ms)`);

      setTimeout(() => {
        console.log(`Stopping stream after ${durationMs} ms`);
