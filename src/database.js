...
db.serialize(() => {
  ...
  // Tabel stream_history: menyimpan riwayat stream
  db.run(`
...
const getHistoryStreamContainers = (callback) => {
  const query = `
    SELECT sh.id, sh.stream_key, sh.title, sh.video_path, COALESCE(sc.stream_url, '') AS stream_url, sh.created_at
    FROM stream_history sh
    LEFT JOIN stream_containers sc ON sh.stream_key = sc.stream_key
...
// Menghapus riwayat stream berdasarkan ID.
const deleteStreamHistory = (id, callback) => {
  db.run('DELETE FROM stream_history WHERE id = ?', [id], callback);
};
...
/**
 * Add a stream history entry to the database.
 * @param {Object} history - The stream history data.
 * @param {string} history.stream_key - The stream key.
 * @param {string} history.title - The title of the stream.
 * @param {string} history.videoPath - The path to the video file.
 * @param {Function} callback - The callback function.
 */
function addStreamHistory({ stream_key, title, videoPath }, callback) {
  const query = `
    INSERT INTO stream_history (stream_key, title, video_path, created_at)
    VALUES (?, ?, ?, datetime('now'))
  `;
  db.run(query, [stream_key, title, videoPath], (err) => {
